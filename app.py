...
.block-container{max-width:1200px;padding:1rem 1.2rem 2rem;}
h1{font-size:1.65rem!important;margin:.1rem 0 .2rem!important;}
h2{font-size:1.15rem!important;margin:.7rem 0 .45rem!important;}
h3{font-size:1rem!important;}
[data-testid="stCaptionContainer"]{font-size:.78rem;}
[data-testid="stMetric"]{padding:.45rem .6rem;border:1px solid rgba(128,128,128,.22);border-radius:10px;}
[data-testid="stMetricLabel"]{font-size:.72rem;}
[data-testid="stMetricValue"]{font-size:1rem;}
[data-testid="stDataFrame"]{border-radius:10px;overflow:hidden;}
[data-testid="stVerticalBlockBorderWrapper"]{border-radius:12px;}
div.stButton>button, div.stFormSubmitButton>button{border-radius:9px;min-height:2.2rem;}
.ai-shell{border:1px solid rgba(128,128,128,.28);border-radius:12px;padding:.45rem;background:rgba(20,20,25,.12);}
.ai-history{border-radius:10px;}
.compact-note{font-size:.76rem;opacity:.72;margin:.1rem 0 .45rem;}
@media (max-width:700px){.block-container{padding:.7rem .55rem 1.5rem;}h1{font-size:1.35rem!important;}.ai-shell{padding:.3rem;}}
</style>
...
st.markdown('<div class="compact-note">Engine Python local → lá số chuẩn hóa → AI luận giải</div>',unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    with st.form("lap_la_so_form"):
        ...
        with c1:
            lich=st.radio("Lịch",["Dương lịch","Âm lịch"],horizontal=True)
        ...
        with c3:
            labels=["Tý (23:00–00:59)","Sửu (01:00–02:59)","Dần (03:00–04:59)","Mão (05:00–06:59)","Thìn (07:00–08:59)","Tỵ (09:00–10:59)","Ngọ (11:00–12:59)","Mùi (13:00–14:59)","Thân (15:00–16:59)","Dậu (17:00–18:59)","Tuất (19:00–20:59)","Hợi (21:00–22:59)"]
...
if lap:
    try:
        ...
        st.session_state.chart_json=normalize_engine_chart(chart)
...
if st.session_state.chart_json:
    ...
    with tabs[0]:
        with st.expander("Xem chi tiết thiên bàn",expanded=False): st.json(tb)
    with tabs[1]:
        rows=[]
        def star_label(x):
            if not isinstance(x,dict): return ""
            ten_sao=x.get("ten",""); dt=x.get("dac_tinh")
        ...
        for name,d in chart.get("12_cung",{}).items():
            ...
            flags=d.get("tuan_triet") or ", ".join(x for x,ok in [("Tuần",d.get("tuan")),("Triệt",d.get("triet"))] if ok) or "—"
            chinh_tinh=d.get("chinh_tinh",[]); phu_tinh=d.get("phu_tinh",[])
            main_ids=set(range(1,15)); main_names={x.get("ten") for x in chinh_tinh if isinstance(x,dict)}; seen=set(); clean_phu=[]
            for x in phu_tinh:
                if not isinstance(x,dict) or not x.get("ten"): continue
                if x.get("id") in main_ids or x.get("ten") in main_names: continue
        ...
        st.dataframe(rows,use_container_width=True,hide_index=True,height=470)
        st.markdown('<div class="compact-note">M = Miếu · V = Vượng · Đ = Đắc · B = Bình · H = Hãm</div>',unsafe_allow_html=True)
    with tabs[2]:
        with st.expander("Đại vận / Tiểu vận",expanded=False):
    ...
    st.markdown("**③ Chat AI**")
    st.markdown('<div class="compact-note">Lịch sử cuộn riêng; ô nhập luôn nằm dưới cùng của khung chat.</div>',unsafe_allow_html=True)
    with st.container(border=True):
        with st.container(height=400,border=False):
            if not st.session_state.chat_history:
                st.info("Nhập câu hỏi để bắt đầu luận giải.")
            else:
        ...
        with st.form("tuvi_ai_chat_form",clear_on_submit=True):
            ...
            with q1:
                q=st.text_input("Câu hỏi",placeholder="Ví dụ: Luận cung Mệnh và công danh…",label_visibility="collapsed",key="tuvi_ai_question")
            with q2:
    ...
    st.info("Nhập thông tin sinh và bấm LẬP LÁ SỐ để bắt đầu.")
